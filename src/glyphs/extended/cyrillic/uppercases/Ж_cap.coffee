...
exports.glyphs['Ж_cap'] =
	unicode: 'Ж'
	glyphName: 'Ж'
	characterName: 'CYRILLIC CAPITAL LETTER Ж'
	ot:
	...
	parameters:
		spacingLeft: 50 * spacing + 15 + ( 22 / 65 ) * serifWidth
		spacingRight: 50 * spacing + 15 + ( 22 / 65 ) * serifWidth
	tags: [
		'all',
		'cyrillic',
		'uppercase'
	...
	contours:
		0:
			...
			nodes:
				0:
					x: spacingLeft + (27/80) * thickness * opticThickness
					y: Math.max( 0, serifHeight * serifArc )
					typeOut: 'line'
					expand:
						width: Math.PI - ( 82 / 80 ) * thickness * opticThickness / Math.cos( Utils.lineAngle(
						        { x: contours[0].nodes[0].x + 0.75 * ( 110 / 80 ) * thickness * opticThickness, y: contours[0].nodes[0].y },
						        { x: contours[0].nodes[0].x + ( contours[1].nodes[0].x - contours[0].nodes[0].x ) * 0.5  - ( ( 90 / 80 ) * thickness * opticThickness ) / 2, y: contours[0].nodes[1].y }
						    ) - Math.PI / 2 )
						angle: Math.PI
						distr: 0.25
				1:
					...
					expand:
						...
						angle: Utils.lineAngle(
						        { x: contours[2].nodes[0].expandedTo[0].x, y: ( 338 / 660 ) * capHeight },
						        { x: contours[3].nodes[1].expandedTo[1].x, y: capHeight }
						    ) - Math.PI
						distr: 1
		1:
			skeleton: true
			...
			nodes:
				0:
					x: Math.max(
						contours[0].nodes[0].x - 0.25 * ( 120 / 80 ) * thickness * opticThickness + 735 + 200 * width - (27),
						contours[0].nodes[0].x + 0.5 * ( 120 / 80 ) * thickness * opticThickness + ( 90 / 80 ) * thickness * opticThickness + 2 * 30 + 0.5 * ( 120 / 80 ) * thickness * opticThickness
					)
					y: Math.max( 0, serifHeight * serifArc )
					typeOut: 'line'
					expand:
				...
				1:
					...
					expand:
						...
						angle: Utils.lineAngle(
						        { x: contours[2].nodes[0].expandedTo[1].x, y: ( 338 / 660 ) * capHeight },
						        { x: contours[4].nodes[1].expandedTo[0].x, y: capHeight }
						    ) - Math.PI
						distr: 1
		2:
			skeleton: true
			...
			nodes:
				0:
					...
					expand:
						...
						distr: 0
				1:
					x: contours[0].nodes[0].expandedTo[0].x + ( 45 / 80 ) * thickness * opticThickness
					y: capHeight
					typeIn: 'line'
					expand:
						width: ( 74 / 80 ) * thickness * opticThickness * Math.sqrt( width ) * contrast
						angle: Math.PI
						distr: 0.75
		4:
			skeleton: true
			...
			nodes:
				0:
					...
					expand:
						...
						distr: 1
				1:
					x: contours[1].nodes[0].expandedTo[0].x - ( 55 / 80 ) * thickness * opticThickness
					y: capHeight
					typeIn: 'line'
					expand:
						width: ( 74 / 80 ) * thickness * opticThickness * Math.sqrt( width ) * contrast
