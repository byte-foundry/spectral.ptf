exports.glyphs['Щ_cap'] =
	unicode: 'Щ'
	glyphName: 'Щ'
	characterName: 'CYRILLIC CAPITAL LETTER Щ'
	ot:
	...
	contours:
		...
		1:
			...
			nodes:
				0:
					x: Math.max(
						contours[0].nodes[0].expandedTo[0].x + 570 + 200 * width - (22),
						contours[0].nodes[0].expandedTo[1].x + 0.75 * ( 90 / 80 ) * thickness * opticThickness + ( 90 / 80 ) * thickness * opticThickness + 10 * 2
					)
					y: Math.max( 0, serifHeight * serifArc )
					typeOut: 'line'
					expand:
